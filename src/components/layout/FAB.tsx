...
export function FAB() {
  ...
  const onClick = () => {
    const perm = localStorage.getItem("restock_camera_permission");
    if (perm === "denied") {
      toast.error("Enable camera in browser settings");
      return;
    }
    setOpen(true);
  };
  ...
  return (
    <>
      ...
      {open && <ScannerModal onScan={handleScan} onClose={() => setOpen(false)} />}
